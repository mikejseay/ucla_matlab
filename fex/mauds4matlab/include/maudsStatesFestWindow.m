function [data, dw, dd, hmfast, hmslow] = maudsStatesFestWindow(data,f,slowwindowsize,fastwindowsize,startsec,endsec)
...
%     end
    ...
    % EMAs cut
    % for this sample, check whether fast > slow (1) or slow > fast (0)
    sign = mfast>mslow;
    
    % if the two crossed
    if(lastsignEMAs ~= sign)
        
        % c is an alternate counter that starts from i and moves either
        % backward or forward in time (depending on the slope at that point)
        % to find a better estimate of the actual transition based on slope
        c = i;
