function [ups, downs] = find_upstates_ema_crossover8_mean(v, dt, slow_width, fast_width, ...
...
% v_slow = movmean_exp(v, round(fast_width_pts / 2));
% v_fast = movmean_exp(v, round(slow_width_pts / 2));
...
small_downs = down_areas < down_area_thresh;
big_ups = up_areas > up_area_thresh;

% use a while loop

% delete downs in between big ups!
delete_downs = small_downs(1:end - 1) & big_ups(1:end - 1) & big_ups(2:end);

downs([delete_downs false]) = [];
ups([false delete_downs]) = [];

% assess remaining up regions
% delete up regions that are too small in area
% delete current up and down indices
