...
%%% this function sets up a GUIDE gui structure and names it gui_State,
%%% which allows us to use certain built-in functions such as gui_mainfcn
%%% to run the GUI element and use its associated code

%%% its input argument could be a string naming a certain callback func
%%% its output could be assigned to some variable

gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...  % names the GUI figure to use, which happens to have the same name as the m-file
...
if nargout
    ...
    % run the "main" function loads the GUI, runs the opening function,
    % then runs the output function
    gui_mainfcn(gui_State, varargin{:}); % note the varargin unloads the args
end
...
%%% GRAPHICS STUFF
UpdateStep = 100;   %100 Updates graphics every X time steps (ms)
NumLineSegs = 10;   %10 Total of Segements shown
numExPlot   = 10;   %10 num Recurrent Units to plot
...
% --- Executes on button press in btIN2.
function btRandInp_Callback(hObject, eventdata, handles)
global InRand InputDur WInEx numEx
...
% --- Executes on button press in btRandInp.
% hObject    handle to btRandInp (see GCBO)
